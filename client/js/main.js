...
window.addEventListener('DOMContentLoaded', () => {
	...
	// 密码验证逻辑 / Password verification logic
	const passwordForm = $id('password-form');
	const passwordError = $id('password-error');
	const accessPassword = $id('access-password');

	// 这里可以修改为你想要的密码
	const CORRECT_PASSWORD = 'nodecrypt';

	passwordForm.addEventListener('submit', (e) => {
		e.preventDefault();
		
		const enteredPassword = accessPassword.value.trim();
		
		if (enteredPassword === CORRECT_PASSWORD) {
			// 密码正确，显示登录页面
			document.body.classList.add('password-verified');
